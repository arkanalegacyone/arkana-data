...
function page5_render(){
  ...
  // Tabs data
  var tabs = [
    { id:"common", label:"Common Powers", content:renderList("Common Powers", commonPowersForRace(race), M.picks) },
    { id:"perks", label:"Perks", content:renderList("Perks", perksForRace(race, arch), M.picks) },
    { id:"archetype", label:"Archetype Powers", content:renderList("Archetype Powers", archPowersForRaceArch(race, arch), M.picks) },
    { id:"cyber", label:"Cybernetics & Hacking", content:(function(){
      var cyberSlots = M.cyberSlots || 0;
    ...
    { id:"magic", label:"Magic Schools & Weaves", content:(function(){
      var groupedMagicSchools = magicSchoolsAllGrouped(race, arch);
      if (!canMagic)
        return '<h3>Magic Schools & Weaves</h3><div class="muted">Not available for '+esc(race)+(arch?' ('+esc(arch)+')':'')+'.</div>';
      return Object.keys(groupedMagicSchools).length
  ...
  // Track active tab - store in window so it persists between renders
  var activeTab = window.arkWizardTab || "common";
  var tabButtons = tabs.map(tab=>
    `<button class="ark-tab-btn${activeTab===tab.id?' active':''}" data-tab="${tab.id}" type="button">${esc(tab.label)}</button>`
  ).join('');
  var tabContent = tabs.find(tab=>tab.id===activeTab)?.content || tabs[0].content;
  ...
  var html =
    ...
    `<div class="ark-tab-bar">${tabButtons}</div>` +
    `<div class="ark-tab-content">${tabContent}</div>` +
    `<div class="ark-nav">
...
// --- Page 5 wiring for tabs ---
function page5_wire(){
  // Tab switching
  Array.prototype.forEach.call(document.querySelectorAll('.ark-tab-btn'), function(btn){
    btn.onclick = function(){
      window.arkWizardTab = btn.getAttribute('data-tab');
      render();
    };
  });

  // Cybernetic slots logic (only present on cyber tab)
